...
add_filter( 'wp_revisions_to_keep', 'my_revisions_to_keep' );
	function my_revisions_to_keep( $revisions ) {
		error_log("set revisions filter: ".$revisions);
		return 2;
}

my_revisions_to_keep(2);
...
add_action('init', 'register_post_types');
	function register_post_types(){
		register_post_type('post_type_name', array(
			'label'  => null,
			'labels' => array(
				'name'               => 'MyHomeWorkTaxonomy', // основное название для типа записи
...
//задаем название для произвольной таксономии Topics для ваших записей
	...
	// также задаем перевод для интерфейса
	  ...
	  $labels = array(
		'name' => _x( 'Home Work Topics', 'Home Work taxonomy name' ),
		...
		'add_new_item' => __( 'Add New Home Work Topic' ),
		'new_item_name' => __( 'New Topic Name' ),
		'menu_name' => __( 'Home Work Topics' ),
	  );
